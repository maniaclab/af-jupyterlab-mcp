...
```bash
helm install jupyterlab-mcp charts/jupyterlab-mcp \
  --namespace mcp \
...
broker:
  identityProviders:
    - type: broker-issued
      alias: jupyterlab-mcp
      displayName: "AF-native services"
      targets: ["jupyterlab-mcp"]
      targetOptions:
        jupyterlab-mcp: { audience: "jupyterlab-mcp", includePosix: true }

aggregator:
  backends:
    - name: jupyterlab-mcp
      prefix: jupyterlab
      url: "http://jupyterlab-mcp.mcp.svc.cluster.local:80/mcp"
      transport: http
