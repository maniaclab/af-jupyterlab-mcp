...
jupyterlab-mcp is an AF-native backend (``type: broker-issued``, ``auth_type:
bearer``, ``audience: jupyterlab-mcp``, ``include_posix: true``): the broker
...
shared-secret mode -- jupyterlab-mcp has no non-broker auth mode).
"""
...
MISSING_AF_CREDENTIALS_MSG = "jupyterlab-mcp requires the af-credentials package: pip install af-credentials[mcp]"
